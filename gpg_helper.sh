...
COMMANDS:
    encrypt_file:   encrypt [FILE] for all public keys in [DIR]
    encrypt_string: encrypt [FILE] for all public keys in [DIR]
...
create_keyring()
{
  ...
  FPRS=`gpg --no-default-keyring --keyring $KEYRING --list-keys --fingerprint --with-colons | grep -A1 fpr | cut -d ':' -f10,21`
