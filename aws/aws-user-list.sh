...
if [ "$1" != "" ]; then
    ...
    profiles=$(grep -oE '\[aws-.*?\]' ~/.aws/credentials | grep -Ev 'aws-dmz-mfa' | tr -d '[]')
fi
...
function days () {
  if [ $1 == null ]; then
    ...
    days="$(( ($(date +%s)-$(date -j -f "%Y-%m-%dT%H:%M:%SZ" "$1" +%s)) / (60*60*24) )) days"
  fi
  echo "${days}"
}

echo "<head> <script src='https://kryogenix.org/code/browser/sorttable/sorttable.js'></script><script src='searchtable.js'></script><link rel='stylesheet' href='https://unpkg.com/purecss@1.0.0/build/pure-min.css' integrity='sha384-nn4HPE8lTHyVtfCBi5yW9d20FjT8BJwUXyWZT9InLYax14RDjBj46LmSztkmNP9w' crossorigin='anonymous'></head>"
